# Generated by CMake 2.8.10.20121204-g9d95d

IF("${CMAKE_MAJOR_VERSION}.${CMAKE_MINOR_VERSION}" LESS 2.5)
   MESSAGE(FATAL_ERROR "CMake >= 2.6.0 required")
ENDIF("${CMAKE_MAJOR_VERSION}.${CMAKE_MINOR_VERSION}" LESS 2.5)
CMAKE_POLICY(PUSH)
CMAKE_POLICY(VERSION 2.6)
#----------------------------------------------------------------
# Generated CMake target import file.
#----------------------------------------------------------------

# Commands may need to know the format version.
SET(CMAKE_IMPORT_FILE_VERSION 1)

# Create imported target dynamorio
ADD_LIBRARY(dynamorio SHARED IMPORTED)

# Create imported target drinjectlib
ADD_LIBRARY(drinjectlib SHARED IMPORTED)

# Create imported target drdecode
ADD_LIBRARY(drdecode STATIC IMPORTED)

# Create imported target drconfiglib
ADD_LIBRARY(drconfiglib SHARED IMPORTED)

# Create imported target drcontainers
ADD_LIBRARY(drcontainers STATIC IMPORTED)

# Create imported target drutil
ADD_LIBRARY(drutil SHARED IMPORTED)

# Create imported target drutil_static
ADD_LIBRARY(drutil_static STATIC IMPORTED)

# Create imported target drgui
ADD_EXECUTABLE(drgui IMPORTED)

# Create imported target drsyms
ADD_LIBRARY(drsyms SHARED IMPORTED)

# Create imported target drsyms_static
ADD_LIBRARY(drsyms_static STATIC IMPORTED)

# Create imported target drwrap
ADD_LIBRARY(drwrap SHARED IMPORTED)

# Create imported target drwrap_static
ADD_LIBRARY(drwrap_static STATIC IMPORTED)

# Create imported target drx
ADD_LIBRARY(drx SHARED IMPORTED)

# Create imported target drx_static
ADD_LIBRARY(drx_static STATIC IMPORTED)

# Create imported target drmgr
ADD_LIBRARY(drmgr SHARED IMPORTED)

# Create imported target drmgr_static
ADD_LIBRARY(drmgr_static STATIC IMPORTED)

# Create imported target drltrace
ADD_LIBRARY(drltrace SHARED IMPORTED)

# Create imported target drcov
ADD_LIBRARY(drcov SHARED IMPORTED)

# Create imported target drcov2lcov
ADD_EXECUTABLE(drcov2lcov IMPORTED)

# Load information for each installed configuration.
GET_FILENAME_COMPONENT(_DIR "${CMAKE_CURRENT_LIST_FILE}" PATH)
FILE(GLOB CONFIG_FILES "${_DIR}/DynamoRIOTarget32-*.cmake")
FOREACH(f ${CONFIG_FILES})
  INCLUDE(${f})
ENDFOREACH(f)

# Commands beyond this point should not need to know the version.
SET(CMAKE_IMPORT_FILE_VERSION)
CMAKE_POLICY(POP)

SET_PROPERTY(TARGET dynamorio PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET dynamorio PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drinjectlib PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drinjectlib PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drconfiglib PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drconfiglib PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drcontainers PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drcontainers PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drutil PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drutil PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drutil_static PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drutil_static PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drgui PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drgui PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drsyms PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drsyms PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drsyms_static PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drsyms_static PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drwrap PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drwrap PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drwrap_static PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drwrap_static PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drx PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drx PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drx_static PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drx_static PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drmgr PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drmgr PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drmgr_static PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drmgr_static PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drltrace PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drltrace PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drcov PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drcov PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET dynamorio PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET dynamorio PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drinjectlib PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drinjectlib PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drconfiglib PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drconfiglib PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drcontainers PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drcontainers PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drutil PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drutil PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drutil_static PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drutil_static PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drgui PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drgui PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drsyms PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drsyms PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drsyms_static PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drsyms_static PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drwrap PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drwrap PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drwrap_static PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drwrap_static PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drx PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drx PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drx_static PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drx_static PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drmgr PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drmgr PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drmgr_static PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drmgr_static PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drltrace PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drltrace PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)

SET_PROPERTY(TARGET drcov PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO Release)
SET_PROPERTY(TARGET drcov PROPERTY MAP_IMPORTED_CONFIG_RELMINSIZE Release)
